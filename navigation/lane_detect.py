    ...
    def detect(self, frame):
        ...
        if lines is not None and len(lines) > 0 and len(lines[0]) > 0:
            ...
            if left_bound is not None:
                scaled = self._scale_line(left_bound[0], left_bound[1],
                                          left_bound[2], left_bound[3],
                                          frame.shape[0])
                left_bound = tuple(int(v) for v in scaled)
            if right_bound is not None:
                scaled = self._scale_line(right_bound[0], right_bound[1],
                                          right_bound[2], right_bound[3],
                                          frame.shape[0])
                right_bound = tuple(int(v) for v in scaled)
